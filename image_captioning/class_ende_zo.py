...
import opts
from classLSTMCoreZo import LSTMCoreZoneout
...
class EncoderDecoder(nn.Module):
    def __init__(self, opt):
        ...
        self.LSTMCore = LSTMCoreZoneout(self.input_encoding_size,
                                        self.lstm_size,
                                        self.drop_prob_lm,
                                        self.c_ratio,
                                        self.h_ratio)
    ...
    def forward(self, fc_feats, seq, is_training):
        ...
        for i in range(seq.size(1)):
            ...
            output, state = self.LSTMCore.forward(xt, state, is_training)
    ...
    def sample_beam(self, fc_feats, init_index, is_training, opt={}):
    ...
    def sample(self, fc_feats, init_index, is_training, opt={}):
        beam_size = opt.get('beam_size', 1)

        if beam_size > 1:
            return self.sample_beam(fc_feats, init_index, is_training, opt)
