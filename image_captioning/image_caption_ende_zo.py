...
def train_xe(opt):
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_images_names), int(opt.batch_size/opt.seq_per_img)),
            ...
            criterion_input = model.forward(current_feats_fc_cuda, current_gt_sents_cuda, True)
        ...
        if np.mod(epoch, 1) == 0:
            ...
            for idx, image_name in enumerate(val_images_names):
                ...
                greedy_seq, greedy_seq_seqLogprobs, greedy_seq_logprobs_all = model.sample(img_feat_fc_cuda, word_to_idx['BOS'], False, {'sample_max': 1})
...
def inference(opt):
    ...
    for idx, image_name in enumerate(infer_images_names):
        ...
        greedy_seq, greedy_seqLogprobs, greedy_logprobs_all = model.sample(img_feat_conv_fc, word_to_idx['BOS'], False, {'sample_max': 1})
...
def beam_search(opt):
    ...
    for idx, image_name in enumerate(beam_images_names):
        ...
        best_sent, best_sent_Logprobs, top_sents, top_sents_Logprobs = model.sample(img_feat_fc_cuda, word_to_idx['BOS'], False, {'beam_size': 3})
