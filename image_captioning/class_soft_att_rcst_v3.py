...
class ShowAttendTellModel(nn.Module):
    def __init__(self, opt):
        ...
        self.rcstLSTM = LSTMCore(self.input_encoding_size, self.lstm_size, self.drop_prob_lm)
        self.h_2_pre_h = nn.Linear(self.lstm_size, self.input_encoding_size)
    ...
    def rcst_forward(self, fc_feats, att_feats, seq, mask):
        ...
        for i in range(seq.size(1) - 1):
            ...
            if i >= self.rcst_time:
                ...
                rcst_h = F.leaky_relu(self.h_2_pre_h(rcst_output)).unsqueeze(1).expand(batch_size, i, self.lstm_size).contiguous().view(-1, self.lstm_size)
                rcst_t = torch.cat([_.unsqueeze(1) for _ in pre_h], 1).view(-1, self.lstm_size).contiguous().detach()

                rcst_mask = mask[:, i].contiguous().view(batch_size, -1).expand(-1, self.lstm_size).unsqueeze(1).expand(batch_size, i, self.lstm_size).contiguous().view(-1, self.lstm_size)
                rcst_loss += torch.mul(nn.MSELoss(reduce=False)(rcst_h, rcst_t), rcst_mask).sum() / (batch_size * i) * self.rcst_scale
