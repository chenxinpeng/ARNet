...
class ShowAttendTellModel(nn.Module):
    def __init__(self, opt):
        ...
        self.rcstLSTM = LSTMCore(self.lstm_size, self.lstm_size, self.drop_prob_lm)  # ARNet is realized by LSTM network
        self.h_2_pre_h = nn.Linear(self.lstm_size, self.lstm_size)  # fully connected layer in ARNet
    ...
    def rcst_forward(self, fc_feats, att_feats, seq, mask):
        ...
        for i in range(seq.size(1) - 1):
            ...
            if i >= 1:
                ...
                rcst_mask = mask[:, i].contiguous().view(batch_size, -1).expand(batch_size, self.lstm_size)
