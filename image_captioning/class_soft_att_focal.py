...
class ShowAttendTellModel(nn.Module):
    def __init__(self, opt, alpha=None):
        ...
        # focal loss
        self.gamma = opt.focal_gamma
        if alpha is None:
            self.alpha = Variable(torch.ones(opt.batch_size, self.vocab_size))
        else:
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        if fc_feats.is_cuda and not self.alpha.is_cuda:
        ...
        for i in range(seq.size(1)-1):
            ...
            # focal loss
            vocab_probs = F.softmax(self.logit(output.squeeze(0)))
            vocab_probs_log = vocab_probs.log()
            vocab_probs_focal = self.alpha * (torch.pow((1 - vocab_probs), self.gamma)) * vocab_probs_log
            outputs.append(vocab_probs_focal)

        # e.g. batch * 19 * vocab_size
        vocab_probs_focal_all = torch.cat([_.unsqueeze(1) for _ in outputs], 1).contiguous()

        return vocab_probs_focal_all
...
if __name__ == '__main__':
    ...
    att_feats = Variable(torch.randn(opt.batch_size, opt.conv_att_size, opt.conv_feat_size))
    ...
    out = model.forward(fc_feats, att_feats, seq)

    sampled_seq, sampled_prob, top_seq, top_prob = model.sample(fc_feats, att_feats, {'beam_size': 3})
