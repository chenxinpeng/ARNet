...
class ShowAttendTellModel(nn.Module):
    def __init__(self, opt):
        ...
        self.rcst_scale = opt.reconstruct_weight
        self.h_2_pre_h_1 = nn.Linear(self.lstm_size, self.lstm_size)
        self.h_2_pre_h_2 = nn.Linear(self.lstm_size, self.lstm_size)
    ...
    def init_weights(self):
        ...
        self.h_2_pre_h_1.weight.data.uniform_(-initrange, initrange)
        self.h_2_pre_h_2.weight.data.uniform_(-initrange, initrange)
        self.h_2_pre_h_1.bias.data.fill_(0)
        self.h_2_pre_h_2.bias.data.fill_(0)
    ...
    # reconstruct 部分
    def forward_rcst(self, fc_feats, att_feats, seq, mask):
        batch_size = fc_feats.size(0)
        ...
        for i in range(seq.size(1) - 1):
            ...
            if i >= self.rcst_time:
                # rcst_output, rcst_state = self.rcstLSTM.forward(output, rcst_state)
                rcst_h_1 = F.leaky_relu(self.h_2_pre_h_1(output))
                rcst_h = F.leaky_relu(self.h_2_pre_h_2(rcst_h_1))

                # rcst_h = self.h_2_pre_h(rcst_h_2)  # input
                rcst_t = pre_h[i - self.rcst_time].squeeze(dim=0).detach()  # target
                ...
                rcst_loss += torch.mul(nn.MSELoss(reduce=False)(rcst_h, rcst_t), rcst_mask).sum() / batch_size * self.rcst_scale
