...
def beam_search(opt, beam_images_names, beam_size=3):
    ...
    model_0.load_state_dict(torch.load(opt.ensemble_model_0))
    model_1.load_state_dict(torch.load(opt.ensemble_model_1))
    model_2.load_state_dict(torch.load(opt.ensemble_model_2))
    model_3.load_state_dict(torch.load(opt.ensemble_model_3))
    model_4.load_state_dict(torch.load(opt.ensemble_model_4))
    ...
    for idx, image_name in enumerate(beam_images_names):
        ...
        for t in range(opt.seq_length + 1):
            ...
            logit = (logits[0] + logits[1] + logits[2]) / 3.0
