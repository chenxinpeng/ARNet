...
from class_ende_rcst_1st import *
...
def train_xe(opt):
    ...
    for epoch in range(opt.max_epochs):
        ...
        if np.mod(epoch, 1) == 0:
            ...
            Bleu_1, Bleu_2, Bleu_3, Bleu_4, CIDEr, METEOR, ROUGE_L = evaluate(opt.train_json_path, val_images_names, val_images_sents)
...
def reconstruct(opt):
    ...
    if os.path.isdir(opt.reconstruct_model_save_path) is False:
        os.mkdir(opt.reconstruct_model_save_path)
    ...
    model.load_state_dict(torch.load(opt.reconstruct_model_path))
    ...
    current_learning_rate = opt.reconstruct_learning_rate
    if opt.optim == 'adam':
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_images_names), opt.batch_size),
                              range(opt.batch_size, len(train_images_names), opt.batch_size)):
            ...
            criterion_input, reconstruct_loss = model.reconstruct_forward(current_feats_fc_cuda, current_gt_sents_cuda, current_gt_masks_cuda)

            xe_loss = criterion.forward(criterion_input, current_gt_sents_cuda[:, 1:], current_gt_masks_cuda)

            total_loss = xe_loss + reconstruct_loss
            ...
            time_end = time.time()
            print("idx {}  epoch {}  lr: {:.10f}  total_loss: {:.3f}  xe_loss: {:.3f}  re_loss: {:.3f}  time: {:.3f}".format(start, epoch, current_learning_rate, train_loss, xe_loss.data[0], reconstruct_loss.data[0], time_end - time_start))

        if np.mod(epoch, 1) == 0:
            print("epoch {} is done, saving the model ...".format(epoch))

            parameter_path = os.path.join(opt.reconstruct_model_save_path, 'model_epoch-' + str(epoch) + '.pth')
            torch.save(model.state_dict(), parameter_path)
            print("parameter model saved to {}".format(parameter_path))

            optimizer_path = os.path.join(opt.reconstruct_model_save_path, 'optimizer_epoch-' + str(epoch) + '.pth')
            torch.save(optimizer.state_dict(), optimizer_path)
            print("optimizer model saved to {}".format(optimizer_path))
            ...
            Bleu_1, Bleu_2, Bleu_3, Bleu_4, CIDEr, METEOR, ROUGE_L = evaluate(opt.reconstruct_train_json_path, val_images_names, val_images_sents)
...
def train_scst(opt):
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_images_names), int(opt.scst_batch_size)),
            ...
            time_end = time.time()
            print("iter: {}  epoch: {},  avg_reward: {:.3f},  total_loss: {:.3f}  time: {:.3f}".format(start, epoch, np.mean(rewards[:, 0]), loss_value, time_end - time_start))
        ...
        if np.mod(epoch, 1) == 0:
            ...
            Bleu_1, Bleu_2, Bleu_3, Bleu_4, CIDEr, METEOR, ROUGE_L = evaluate(opt.scst_train_json_path, val_images_names, val_images_sents)
...
def inference(opt):
    ...
    Bleu_1, Bleu_2, Bleu_3, Bleu_4, CIDEr, METEOR, ROUGE_L = evaluate(opt.infer_json_path, infer_images_names, infer_images_sents)
...
def beam_search(opt):
    ...
    Bleu_1, Bleu_2, Bleu_3, Bleu_4, CIDEr, METEOR, ROUGE_L = evaluate(opt.beam_json_path, beam_images_names, beam_images_sents)
