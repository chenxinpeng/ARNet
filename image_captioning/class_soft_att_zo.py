...
import opts
from classLSTMSoftAttCoreZo import LSTMSoftAttentionCoreZoneout
...
class ShowAttendTellModel(nn.Module):
    def __init__(self, opt):
        ...
        # Zoneout, c_ratio, h_ratio
        self.c_ratio = opt.c_ratio
        self.h_ratio = opt.h_ratio

        # 由 fc feature 初始化 LSTM 的 state, (c, h)
        self.fc2h = nn.Linear(self.fc_feat_size, self.lstm_size)

        self.core = LSTMSoftAttentionCoreZoneout(self.input_encoding_size,
                                                 self.lstm_size,
                                                 self.conv_feat_size,
                                                 self.conv_att_size,
    ...
    def forward(self, fc_feats, att_feats, seq, is_training):
        ...
        for i in range(seq.size(1)-1):
            ...
            output, state = self.core.forward(xt, att_feats, state, is_training)
    ...
    def sample_beam(self, fc_feats, att_feats, init_index, is_training, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(self.seq_length + 1):
                ...
                output, state = self.core.forward(xt, att_feats_current, state, is_training=False)
    ...
    def sample(self, fc_feats, att_feats, init_index, is_training, opt={}):
        sample_max = opt.get('sample_max', 1)
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)

        if beam_size > 1:
            return self.sample_beam(fc_feats, att_feats, init_index, is_training, opt)
    ...
    def teacher_forcing_get_hidden_states(self, fc_feats, att_feats, seq, is_training):
        batch_size = fc_feats.size(0)
    ...
    def free_running_get_hidden_states(self, fc_feats, att_feats, init_index, is_training):
        batch_size = fc_feats.size(0)
        seq = []
        seqLogprobs = []
        logprobs_all = []
