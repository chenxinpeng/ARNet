...
class ShowAttendTellModel(nn.Module):
    def __init__(self, opt):
        ...
        self.rcstLSTM = LSTMCore(self.lstm_size, self.lstm_size, self.drop_prob_lm)
    ...
    def forward_rcst(self, fc_feats, att_feats, seq, mask):
        ...
        for i in range(seq.size(1) - 1):
            ...
            if i >= 1:
                ...
                rcst_t = pre_inputs[i - 1].squeeze(dim=0).detach()  # target
                ...
                # rcst_loss += torch.mul(nn.MSELoss(reduce=False)(rcst_h, rcst_t), rcst_mask).sum() / batch_size * self.rcst_scalar
