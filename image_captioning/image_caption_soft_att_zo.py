...
def train_xe(opt, train_images_names, val_images_names):
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_images_names), int(opt.batch_size/opt.seq_per_img)),
            ...
            criterion_input = model.forward(current_feats_fc_cuda, current_feats_conv_cuda, current_gt_sents_cuda, is_training=True)
            ...
            time_end = time.time()
            print("idx: {}  epoch: {}  lr: {:.10f}  c: {}  h: {}  loss: {:.3f}  time: {:.3f}".format(start, epoch, current_learning_rate, opt.c_ratio, opt.h_ratio, train_loss, time_end - time_start))
...
def inference(opt, infer_images_names):
    ...
    for idx, image_name in enumerate(infer_images_names):
        ...
        greedy_seq, greedy_seqLogprobs, greedy_logprobs_all = model.sample(img_feat_conv_fc, img_feat_conv_cuda, word_to_idx['BOS'], False, {'sample_max': 1})
...
def beam_search(opt, beam_images_names):
    ...
    for idx, image_name in enumerate(beam_images_names):
        ...
        best_sent, best_sent_Logprobs, top_sents, top_sents_Logprobs = model.sample(img_feat_conv_fc, img_feat_conv_cuda, word_to_idx['BOS'], False, {'beam_size': 3})
...
def t_SNE_visilization(opt, train_images_names, val_images_names):
    ...
    for start, end in zip(range(0, len(val_images_names), opt.t_SNE_batch_size),
        ...
        # training mode
        model.train()
        current_teacher_forcing_hidden_states = model.teacher_forcing_get_hidden_states(current_feats_fc_cuda, current_feats_conv_cuda, current_gt_sents_cuda, is_training=False)

        # eval model
        model.eval()
        current_free_running_hidden_states = model.free_running_get_hidden_states(current_feats_fc_cuda, current_feats_conv_cuda, word_to_idx['BOS'], is_training=False)
