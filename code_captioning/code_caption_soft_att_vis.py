...
def visualization(opt):
    ...
    model.copy_weights(opt.t_SNE_model_path)
    model.cuda()
    ...
    for start, end in zip(range(0, len(test_data_index), opt.t_SNE_batch_size),
                          range(opt.t_SNE_batch_size, len(test_data_index), opt.t_SNE_batch_size)):
