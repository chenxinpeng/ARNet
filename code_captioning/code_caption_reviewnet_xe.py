...
def reviewnet(opt):
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_data_index), opt.batch_size),
            ...
            decode_logit_seq = model.forward(current_code_matrix_cuda, current_comment_matrix_cuda, current_comment_mask_cuda)
            encode_decode_loss = encode_decode_criterion.forward(decode_logit_seq, current_comment_next_cuda, current_comment_mask_cuda)

            # backward
            encode_decode_loss.backward()
            ...
            iter_idx += 1
            time_end = time.time()
            print("{}  {}  epoch: {}  lr: {:.10f}  ss: {}  loss: {:.3f}  time: {:.3f}".format(iter_idx,
                start, epoch, current_learning_rate, model.ss_prob, encode_decode_loss_val, time_end - time_start))

        if np.mod(epoch, 1) == 0:
            print("\nepoch {} is done, saving the model ...".format(epoch))

            parameter_path = os.path.join(opt.model_save_path, 'model_epoch-' + str(epoch) + '.pth')
            torch.save(model.state_dict(), parameter_path)
            print("\nparameter model saved to {}".format(parameter_path))

            optimizer_path = os.path.join(opt.model_save_path, 'optimizer_epoch-' + str(epoch) + '.pth')
