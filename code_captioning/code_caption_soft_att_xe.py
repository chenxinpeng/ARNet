...
def train_xe(opt):
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_data_index), opt.batch_size),
            ...
            decode_logit_seq = model.forward(current_code_matrix_cuda,
        ...
        if np.mod(epoch, 1) == 0:
            ...
            for start, end in zip(range(0, len(val_data_index), opt.batch_size),
                ...
                greedy_seq, _, _ = model.sample(current_code_matrix_cuda, token2index['BOS'], token2index['EOS'])
