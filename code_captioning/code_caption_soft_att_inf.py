...
def inference(opt):
    ...
    for start, end in zip(range(0, len(test_data_index), opt.batch_size),
                          range(opt.batch_size, len(test_data_index), opt.batch_size)):
    ...
    print("Bleu_1: {:.5f}  Bleu_2: {:.5f}  Bleu_3: {:.5f}  Bleu_4: {:.5f}  METEOR: {:.5f}  CIDEr: {:.5f}  ROUGE: {:.5f}".format(avg_score[0], avg_score[1],
        avg_score[2], avg_score[3], avg_score_meteor, avg_score_cider, avg_score_rouge))
